...
import { MdFolder, MdOutlineCloseFullscreen } from "react-icons/md";
...
const DocumentSelector = ({ setImageId, setImageSrc,isOpen,handleClose }) => {
    
    return (
        <div className='fixed top-0 left-0 -mt-[2px] py-1 flex flex-col   w-full h-[90dvh] 
         rounded-xl shadow-md z-10'>
            <div className="bg-slate-500 h-full w-full   rounded-md  grid grid-rows-[50px_120px_1fr]  ">
                ...
                <DocumentsLoader setImageId={setImageId} setImageSrc={setImageSrc} />
            </div>
...
const DocumentsLoader = ({ setImageId, setImageSrc }) => {
    const docQuery = useDocumentQuery()
    // //run on component mount only
    if (docQuery.documents.isLoading) {
        return <div className="w-full h-full  flex flex-row gap-2 justify-center items-center">
            <h1 className="text-4xl flex flex-row gap-2  "><Loader className='animate-spin' />Loading data...</h1>
        </div>
    }
    if (docQuery.documents.isError) {
        return <div className="w-full h-full  flex flex-row gap-2 justify-center items-center">
            <p className="text-4xl flex flex-row gap-2  ">An error occurred while fetching the data...</p>
        </div > 
    }
    return (
        <>
...
const DocumentSelectConfirm = ({ document, setImageId, setImageSrc }) => {
    const handleOnClick = () => {
