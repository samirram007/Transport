...
import { useFee } from '../hooks/queries';
...
export const PrintForm = ({ feeId }) => {

  const fetchedData = useFee(feeId)

  const feeData = fetchedData.data?.data
    ...
    <>
      ...
      <div className='relative border-2 border-slate-900 mt-2 min-h-[400px]  '>
        <div className=' grid grid-cols-10 px-2 py-1'>
          <div className={' col-span-2 '}>No. {feeData.feeNo}</div>
          <div className={' col-span-6 '}></div>
          <div className={' col-span-2 text-right '}>
            Date: {DateTime.fromISO(feeData.feeDate).toLocaleString(DateTime.DATE_MED)}
          </div>
        </div>
          ...
          <div className='col-span-8 text-center'>
            ...
            <div className='flex flex-col pl-4 '>
              ...
              <div className='flex flex-row gap-6'>
                ...
                <div>RollNo: {feeData.riderSnapshot?.rollNo}</div>
                <div>Time: {capitalizeAllWords(feeData.riderSnapshot?.schoolTime)}</div>
              </div>
        ...
        <div className='px-2  '>
          ...
          <FeeEntryRowsPrint feeItems={feeData.feeItems} />
        ...
        <div className='w-full py-1 pl-4 text-[10px] -mt-2'>
          (in words) :  {convertNumberToWords(Number(Number(feeData.totalAmount).toFixed(2)))}
        </div>
...
export const FeeEntryRowsPrint = ({ feeItems }) => {

  return (
    feeItems && feeItems.map((feeItem, index) => (

      <FeeEntryRowPrint key={index} index={index} feeItem={feeItem} />
...
export const FeeEntryRowPrint = ({ index, feeItem }) => {
      ...
      <div className={` grid grid-cols-12   gap-2 items-center  text-xs    `}>
        ...
        <div className='col-span-10 flex flex-row items-center gap-2 pl-4 border-r-2 border-slate-800'>
          <div className='py-2'>
            <div className='flex flex-row flex-nowrap items-center  '>
              {capitalizeAllWords(feeItem.feeHead.name)}


              <div className={`flex flex-row ${feeItem.feeItemMonths.length > 6 ? 'gap-1 ml-1' : 'gap-2 ml-2'}`}>
                {feeItem.feeItemMonths.map((x, index) => (
                  (<div key={index} className={`
                     border-slate-400  text-slate-800
                   ${feeItem.feeItemMonths.length > 6 ? 'text-xs px-1 border-b-2' : 'text-xs border-2 px-2 rounded-md'}
                    font-bold mb-[1px]  `}>
                    {x.month?.shortName}
                  </div>)
                ))}
        ...
        <div className='col-span-2 text-right '>
          <div className='py-1 pr-4 flex flex-row items-center justify-end'>
            {feeItem.amount > 0 ? Number(parseFloat(feeItem.totalAmount)).toFixed(2) : ''}
          </div>
        </div>
      </div>
    </>
