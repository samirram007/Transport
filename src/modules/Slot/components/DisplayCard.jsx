...
const BaseCard = () => {
  const { selectedSlot: data } = useSlotContext()
  return (
    data &&
     ...
     rounded-lg '>
      ...
      <CardContent className='flex flex-col items-center gap-4 p-0 px-6'>
        ...
        <div className="flex flex-col gap-2  items-start  max-h-[15rem] overflow-y-auto scrollbar-thin scrollbar-thumb-teal-500 scrollbar-track-teal-200">
          ...
          <InfoRow label="Start Time" value={data.startTime} />
          <InfoRow label="End Time" value={data.endTime} />
          <InfoRow label="Capacity" value={data.capacity} />
          
          <InfoRow label="Type" value={data.slotType} />
