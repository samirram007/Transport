...
import { useCampusContext } from "../context/features/useCampusContext"
...
const BaseCard = () => {
  const { selectedCampus: data } = useCampusContext()
  return (
    data &&
     ...
     rounded-lg '>
      ...
      <CardContent className='flex flex-col gap-4 items-center p-0 px-6'>
        ...
        <div className="flex flex-col gap-2  items-start  max-h-[15rem] overflow-y-auto scrollbar-thin scrollbar-thumb-teal-500 scrollbar-track-teal-200">
          ...
          <InfoRow label="Estb. Date" value={moment(data.establishmentDate).format("DD-MM-YYYY")} />
          <InfoRow label="Board" value={data.educationBoard?.name} />
          <InfoRow label="School" value={data.school?.name} />
          <InfoRow label="Address" value={data.address?.display} />
          <InfoRow label="Opening" value={data.opening} />
          <InfoRow label="Closing" value={data.closing} />
