...
import { PrintModal } from '../ExpenseProcess/print/PrintToPdf';


const Filter = lazy(() => import('./Filter'))
const EditExpenses = lazy(() => import('../ExpenseProcess/EditExpenses'))
...
export default function ExpenseTable({ data, columns, pageSize = 2000, createRoute,
        ...
        <div className='container-flex md-container'>
...

            {
                ...
                <div>
                    ...
                    {
                        (!ExpenseData.data || ExpenseData.isError || ExpenseData.data.data.length === 0)
                                ...
                                <div className='flex flex-row justify-center  rounded-lg text-slate-200 items-center  px-4'>
                                    ...
                                    <div className='relative w-[100%] max-h-[55dvh] 2xl:max-h-[70dvh] overflow-y-auto overflow-x-hidden pb-2 flex gap-2 flex-col flex-nowrap justify-between my-2'>
                                        {table.getRowModel().rows.map((row, index) => (
                                            <ExpenseCard row={row} index={row.id} key={index} />
                                        ))}
...
export const ExpenseCard = ({ row, index }) => {
        ...
        <div className='flex w-100 flex-row flex-nowrap bg-blue-600/10 p-2 rounded-lg gap-2'>
            <div className='flex flex-col gap-2 w-1/2'>
                <div className='badge badge-success'>
                    <span className='w-16'>Expense No:</span>  <span>{data.expense_no}</span>
                </div>
                <div>

                    <div className='px-2'>
                        <span className='w-16'>Date: </span>{DateTime.fromISO(data.expense_date).toLocaleString(DateTime.DATE_MED)}
                    </div>
                    <div className='px-2'>
...
export const ExpenseDetails = ({ data }) => {
    ...
    return (
        ...
        gap-1   border-b m-0  pb-1'>
                ...
                <div className='relative flex flex-row flex-nowrap gap-2'>
                    <PrintToPdf expenses={data}
                        session_id={data.student_session.academic_session_id} />
                    <ExpenseItems expenses={data} selectedStudentSession={data.student_session} />
        ...
        gap-1   border-b mb-1 bg-slate-400 text-slate-800 font-bold text-[8px]'>
            ...
            {data && data.expense_items.map((item, index) => (
                <div key={index} className='text-xs/[12px] p-1 flex flex-row flex-nowrap justify-between items-center
                    ...
                    <div className='flex flex-row gap-2 '>
                        {item.expense_head.name}
                        {
                            item.expense_item_months &&
                            item.expense_item_months.length > 0 &&
                            item.expense_item_months.map((x, index) => (
                                <div key={index} className='underline decoration-blue-400 text-green-600 text-xs'>{x.month?.short_name}</div>
                            ))
                        }
                        {/* ( item.expense_item_months.length>0 && item.expense_item_months.map(x=>x.month.short_name))} */}
...
export const ExpenseItems = ({ expenses, selectedStudentSession }) => {
    ...
    return (
        <>
            ...
            </span>
            {
                isOpen &&
                <>
                    <FormikEditFormModal isOpen={isOpen} setOpen={setOpen} label="Edit Expenses">
                        <EditExpenses userData={expenses.student} expenses={expenses} isOpen={isOpen} setOpen={setOpen} selectedStudentSession={selectedStudentSession} />
