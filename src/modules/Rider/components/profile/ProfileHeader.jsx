...
import { useGlobalDataContext } from '@/modules/GlobalData/contexts/features/useGlobalDataContext';
...
const ProfileHeaderBackup = () => {
  const {selectedStudent:data}=useStudentContext()
  const {selectedAcademicSession}=useGlobalDataContext()
  const academicSessionsData=useAcademicSessions()
  const fetchedAcademicSessions=academicSessionsData.data?.data
...
shadow-lg border-b-[1px] border-slate-50/20 text-red-300 font-bold -mb-2  `
  ...
  const [studentSessions, setStudentSessions] = useState(data?.studentSessions ?? [])

  const [selectedSession, setSelectedSession] = useState(
    studentSessions.length > 0
      ? (
        studentSessions.find(x => x.academicSession.isCurrent == 1)
          ?.academicSession
        ?? studentSessions[0].academicSession
      )
      : null
  )
